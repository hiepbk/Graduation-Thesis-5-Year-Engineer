...
print(test_thu)
Q_peak = np.array(data['Q_peak'])
#print(temperature)
num_periods = 24
f_horizon = 1
print(len(Q_peak))
x_data = Q_peak[:(len(Q_peak)-(num_periods*2))]
print(x_data)
#print(x_data)
x_batches = x_data.reshape(-1, num_periods, 1)
y_data = Q_peak[1:(len(Q_peak)-(num_periods*2))+f_horizon]
y_batches = y_data.reshape(-1, num_periods, 1)
#print(y_batches.shape)
def test_data(series, forecast, num):
    testX = Q_peak[-(num + forecast):][:num].reshape(-1, num_periods, 1)
    testY = Q_peak[-(num):].reshape(-1, num_periods, 1)
    return testX, testY
X_test, Y_test = test_data(Q_peak, f_horizon, 24*2)
...
save_path = saver.save(sess, "models/Q_peak.ckpt")
...
plt.ylabel("Q_peak (MWar)")
plt.show()
with tf.Session() as sess:
  # Restore variables from disk.
    saver.restore(sess, "models/Q_peak.ckpt")

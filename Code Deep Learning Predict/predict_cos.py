...
print(test_thu)
cos = np.array(data['cos'])
#print(temperature)
num_periods = 24
f_horizon = 1
print(len(cos))
x_data = cos[:(len(cos)-(num_periods*2))]
print(x_data)
#print(x_data)
x_batches = x_data.reshape(-1, num_periods, 1)
y_data = cos[1:(len(cos)-(num_periods*2))+f_horizon]
y_batches = y_data.reshape(-1, num_periods, 1)
#print(y_batches.shape)
def test_data(series, forecast, num):
    testX = cos[-(num + forecast):][:num].reshape(-1, num_periods, 1)
    testY = cos[-(num):].reshape(-1, num_periods, 1)
    return testX, testY
X_test, Y_test = test_data(cos, f_horizon, 24*2)
...
save_path = saver.save(sess, "models/cos.ckpt")
...
with tf.Session() as sess:
  # Restore variables from disk.
    saver.restore(sess, "models/cos.ckpt")

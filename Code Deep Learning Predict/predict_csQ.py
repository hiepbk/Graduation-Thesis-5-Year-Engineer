...
f_horizon = 1
print(len(csQ))
x_data = csQ[:(len(csQ)-(num_periods*2))]
print(x_data)
#print(x_data)
x_batches = x_data.reshape(-1, num_periods, 1)
y_data = csQ[1:(len(csQ)-(num_periods*2))+f_horizon]
y_batches = y_data.reshape(-1, num_periods, 1)
#print(y_batches.shape)
def test_data(series, forecast, num):
    testX = csQ[-(num + forecast):][:num].reshape(-1, num_periods, 1)
    testY = csQ[-(num):].reshape(-1, num_periods, 1)
    return testX, testY
X_test, Y_test = test_data(csQ, f_horizon, 24*2)
...
save_path = saver.save(sess, "models/cs_Q.ckpt")
...
plt.ylabel("Công suất(MVar)")
plt.show()
with tf.Session() as sess:
  # Restore variables from disk.
    saver.restore(sess, "models/cs_Q.ckpt")

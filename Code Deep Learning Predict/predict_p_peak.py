...
print(test_thu)
P_peak = np.array(data['P_peak'])
#print(temperature)
num_periods = 24
f_horizon = 1
print(len(P_peak))
x_data = P_peak[:(len(P_peak)-(num_periods*2))]
print(x_data)
#print(x_data)
x_batches = x_data.reshape(-1, num_periods, 1)
y_data = P_peak[1:(len(P_peak)-(num_periods*2))+f_horizon]
y_batches = y_data.reshape(-1, num_periods, 1)
#print(y_batches.shape)
def test_data(series, forecast, num):
    testX = P_peak[-(num + forecast):][:num].reshape(-1, num_periods, 1)
    testY = P_peak[-(num):].reshape(-1, num_periods, 1)
    return testX, testY
X_test, Y_test = test_data(P_peak, f_horizon, 24*2)
...
save_path = saver.save(sess, "models/P_peak.ckpt")
...
plt.ylabel("P_peak (MW)")
plt.show()
with tf.Session() as sess:
  # Restore variables from disk.
    saver.restore(sess, "models/P_peak.ckpt")
